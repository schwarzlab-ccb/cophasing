...
if [ "$#" -ne 4 ]; then
        echo "USAGE: $0 <input_bam_dir_Repl1> <input_bam_dir_Repl2> <binned_genome> <output_dir>"
        exit
fi

indir_bam_Repl1=$1 ## directory with input bam files of GAM experiments
indir_bam_Repl2=$2
binned_genome=$3 ## binned genome with window / bin size showing the resolution of final chromatin contact map
outdir=$4 ## directory to store output

# exit if no input is given
if [ ! -d "$indir_bam_Repl1" ]; then
    echo "directory $indir_bam_Repl1 not found. Exiting."
    exit 1
fi

if [ ! -d "$indir_bam_Repl2" ]; then
    echo "directory $indir_bam_Repl2 not found. Exiting."
    exit 1
fi
...
files1=`find $indir_bam_Repl1 -name '*.bam'` 
files2=`find $indir_bam_Repl2 -name '*.bam'` 
files="${files1} ${files2}"
...
echo "Running analysis with $0 on $indir_bam_Repl1/*.bam and $indir_bam_Repl2/*.bam,  $binned_genome writing to $outdir at $(date +%x_%r)" >> $outdir/run.log
...
for f in $files; do
	...
	bedtools coverage -a $binned_genome -b $f | awk '{print $5}' > $outdir/$b.cov
        ((i++))
done
